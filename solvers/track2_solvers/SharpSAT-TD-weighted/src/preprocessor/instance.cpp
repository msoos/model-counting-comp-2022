...
Instance::Instance(string input_file, bool weighted_) {
	...
	if (weighted) {
		for (int v = 1; v <= vars; v++) {
			if (read_weight[PosLit(v)] && !read_weight[NegLit(v)]){
				weights[NegLit(v)] = (double)1-weights[PosLit(v)];
			}
			if (read_weight[NegLit(v)] && !read_weight[PosLit(v)]){
				weights[PosLit(v)] = (double)1-weights[NegLit(v)];
			}
		}
		for (int v = 1; v <= vars; v++) {
